...
fs.writeFileSync('npm-tests/data.json', JSON.stringify(data));
...
app.get('/',(request,reply)=>{
    ...
    const jsonData = JSON.parse(fs.readFileSync('npm-tests/data.json').toString());

    let list = '';
    jsonData.users.forEach((name) => list += `<p>${name}</p>`);
